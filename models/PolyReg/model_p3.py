...
def test_model_p3(weights, elements, e_neg, ref_e, exp_e):
    '''
    Return the testing loss for Polynomial Model 3 with weights 'weights'
        y = ax^2 + bx + c
        - y := experimental binding energy - atomic orbital energy
        - x := electronegativity environment 
        - a, b, c := weights to train
    Arguments:
        - weights: 1D array of length 'num_element'*3
...
def train_model_p3(elements, e_neg, ref_e, exp_e, num_elements=100):
    ...
    '''
    weights = np.zeros(num_elements*3)
    def errorfunc(x):
        xvec = get_xvec(x.reshape(-1,3), elements)
        arr = np.vstack([e_neg**2, e_neg, np.ones(e_neg.shape)]).T
